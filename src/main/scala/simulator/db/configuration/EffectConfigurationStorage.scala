...
class EffectConfigurationStorage(override val tableName: String) extends Storage {
  ...
  def write(model: EffectConfig, configurationId: UUID) =
    (sql"""INSERT INTO """ ++ tableNameFragment ++
      sql""" (id, configuration_id, name, effect_type, target)
          VALUES (${model.id}, $configurationId, ${model.name}, ${model.effectType}, ${model.target})
